	...
	ulint	n_instances)	/*!< in: number of instances */
{
    ...
    if (srv_use_share && share_fd <= 0) {
		...
		share_int[255][0] = 0; share_int[255][1] = 0;
		ib_logf(IB_LOG_LEVEL_INFO,
				"USE SHARE device: %s\n", srv_share_device);
		memset(share_buf, 0, 4096);
		buf_init_cdb();
		char cmd_buf[1024];
		sprintf(cmd_buf, "sudo nvme vuc %s enable", srv_share_device);
		system(cmd_buf);
    }
    //]]ogh
...
/*
	case BUF_IO_NONE: // ogh : MOVE SHARE
		//buf_flush_write_complete(bpage);
		if (uncompressed) {
			rw_lock_s_unlock_gen(&((buf_block_t*) bpage)->lock,
					     BUF_IO_WRITE);
