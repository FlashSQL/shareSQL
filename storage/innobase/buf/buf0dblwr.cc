				...
				header on trx sys page */
{
	...
	//ogh for share
	mutex_create(buf_dblwr_mutex_key,
		     &buf_dblwr->share_mutex, SYNC_DOUBLEWRITE);
...
/*================*/
{
	...
	mutex_free(&buf_dblwr->share_mutex); //ogh for share
	mem_free(buf_dblwr);
	buf_dblwr = NULL;
}
...
void buf_init_cdb() { 
	...
	memset(share_buf, 0, sizeof(char) * 4096);
...
buf_get_single_lsn(int fd, uint64_t offset) {                                                                  
	...
	if( ioctl(fd, FIBMAP, &ret) ){                                   
		ib_logf(IB_LOG_LEVEL_ERROR,
				"Error while getting lpn");
		ut_error;
	}                                                                

	//ret = ((uint64_t)ret * 8 + START_SECTOR) / 8; //return system-wise sector number
	ret = ((uint64_t)ret * 8 + START_SECTOR); //return system-wise sector number
#if 0
	ib_logf(IB_LOG_LEVEL_INFO,
		"[SHARE] lsn is %lu",
		ret);
...
buf_share_send_command(int dwb_fd) {
	...
	if (srv_use_share == 1) {
		memcpy(share_buf, &share_count, sizeof(share_count));
		if (0 > ioctl(share_fd, SG_IO, &io_hdr)) {
			ib_logf(IB_LOG_LEVEL_ERROR, 
					"Error while sending share command");
...
ulint get_space_id(buf_page_t *page) {
	const buf_block_t* block = (buf_block_t*)page;

	return buf_block_get_space(block);
}

static
void
buf_share_add_to_batch(int db_fd, int dwb_fd,
			uint64_t source, uint64_t target) {
	uint32_t slpn, tlpn;

	slpn = buf_get_single_lsn(db_fd, source);
	tlpn = buf_get_single_lsn(dwb_fd, target);

	if ((slpn % 8) !=0 || (tlpn % 8) !=0) {
		ib_logf(IB_LOG_LEVEL_ERROR,
			"LPN does not match to block boundary, %d, %d\n",
			slpn, tlpn);
	}
	while (share_count == srv_share_max); // wait until send share command
	if (srv_use_share == 1) {
		memcpy(share_buf + 4 + (share_count * 8), &slpn, sizeof(uint32_t));
		memcpy(share_buf + 8 + (share_count * 8), &tlpn, sizeof(uint32_t));
	} else {
		share_int[share_count][0] = slpn;
		share_int[share_count][1] = tlpn;
	}
#if 0
	ib_logf(IB_LOG_LEVEL_INFO,
		"[SHARE ogh] share idx: %d",
		share_count);
#endif
    share_count++;
		...
		for (ulint i = 0 ;i < first_free; i++) {
			...
			if (bpage->zip.data) {
				fil_get_fd_offset( get_space_id(bpage),
						...
						buf_page_get_zip_size(&block->page), &db_fd, &source);
			} else {
				fil_get_fd_offset( get_space_id(bpage),
						buf_block_get_page_no(block), 0, &db_fd, &source);
			}
    ...
    if (srv_use_share) {
		...
		if (bpage->zip.data) {
			fil_get_fd_offset( get_space_id(bpage),
					...
					buf_page_get_zip_size(bpage), &db_fd, &source);
		} else {
			fil_get_fd_offset( get_space_id(bpage),
					buf_block_get_page_no((buf_block_t*)bpage), 0, &db_fd, &source);
		}
...
#if 1
		fil_share_complete_io(get_space_id(bpage),
				buf_block_get_page_no((buf_block_t*)bpage));
		fil_share_complete_io(TRX_SYS_SPACE, offset);
