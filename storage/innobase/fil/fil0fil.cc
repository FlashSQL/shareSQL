    ...
    uint64_t* real_offset)
{
	...
	if (space == 0) {
		...
		ib_logf(IB_LOG_LEVEL_ERROR,
			...
			"not exist. i/o type SHARE, space id %lu, "
			"page no. %lu",
			(ulong) space_id, (ulong) block_offset);
	...
	for (;;) {
		if (node == NULL) {
			ib_logf(IB_LOG_LEVEL_ERROR,
					"Trying to do i/o to a tablespace which does "
					"not exist. i/o type SHARE, space id %lu: %s, "
					"page no. %lu",
					(ulong) space_id, space->name, (ulong) block_offset);
	...
	if (!fil_node_prepare_for_io(node, fil_system, space)) {
		if (space->purpose == FIL_TABLESPACE
			...
			ib_logf(IB_LOG_LEVEL_ERROR,
				...
				"i/o type SHARE, space id %lu, page no %lu, ",
				(ulong) space_id,
				(ulong) block_offset);
	...
	ib_logf(IB_LOG_LEVEL_INFO,
		"File size: %lu, cur-pgno: %lu, cur-offset: %lu",
		node->size, block_offset, offset);
	ib_logf(IB_LOG_LEVEL_INFO,
		"Being-extended: %u", node->being_extended);
#endif
	*fd = node->handle;
	*real_offset = offset;
...
int fil_share_is_table(ulint space_id) 
{
	fil_space_t*	space;
	fil_mutex_enter_and_prepare_for_io(space_id);
	space = fil_space_get_by_id(space_id);
